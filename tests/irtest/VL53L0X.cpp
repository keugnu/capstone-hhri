...
{
  char wreg[2] = {(char)reg, (char)value};
  int file;
  ...
  if (close(file) < 0) 
  {
    printf("Failed to close the bus (writeReg). \n");
    exit(1);
  }
}
...
{  
  ...
  if (close(file) < 0) 
  {
    printf("Failed to close the bus (writeReg16Bit). \n");
    exit(1);
  }
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  ioctl(file, I2C_SLAVE, address);
  char wreg[5] = {reg, 
                 ((value >> 24) & 0xFF),  // value highest byte
                 ((value >> 16) & 0xFF),
                 ((value >>  8) & 0xFF),
                 (value & 0xFF) };        // value lowest byte
  write(file, wreg, 5);
  if (close(file) < 0) 
  {
    printf("Failed to close the bus (writeReg32Bit). \n");
    exit(1);
  }
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  if (close(file) < 0)
  {
    printf("Failed to close the bus (readReg). \n");
    exit(1);
  }
  return value[0];
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  if (close(file) < 0)
  {
    printf("Failed to close the bus (readReg16Bit). \n");
    exit(1);
  }
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  write(file, wreg, 1);
  read(file, i2c_rx_buff, 4);

  if (close(file) < 0)
  {
    printf("Failed to close the bus (readReg32Bit). \n");
    exit(1);
  }

  value  = i2c_rx_buff[0] << 24; // value highest byte
  value |= i2c_rx_buff[1] << 16;
  value |= i2c_rx_buff[2] <<  8;
  value |= i2c_rx_buff[3];       // value lowest byte
