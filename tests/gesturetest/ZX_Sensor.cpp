...
bool ZX_Sensor::wireWriteByte(uint8_t val)
{
    ...
    {
	printf("Failed to open the bus (wireWriteByte). \n");
	exit(1);
    }
    ioctl(file, I2C_SLAVE, addr_);
	
    write(file, wreg, 1);
    
    if (close(file) < 0)
    {
	printf("Failed to close the bus (wireWriteByte). \n");
        return false;
    }
    return true;
...
bool ZX_Sensor::wireWriteDataByte(uint8_t reg, uint8_t val)
{
    ...
    if((file = open(bus, O_RDWR)) < 0)
    {
	printf("Failed to open the bus (wireWriteDataByte). \n");
        exit(1);
    }
    ioctl(file, I2C_SLAVE, addr_);
    ...
    if(close(file) < 0)
    {
	printf("Failed to close the bus (wireWriteDataByte).\n");
	return false;
    }
    return true;
...
bool ZX_Sensor::wireReadDataByte(uint8_t reg, uint8_t &val)
{
    ...
    if ((file = open(bus, O_RDWR)) < 0)
    {
	printf("Failed to open the bus (wireReadDataByte). \n");
	exit(1);
    }
    ioctl(file, I2C_SLAVE, addr_);
    ...
    if(close(file) < 0)
    {
	printf("Failed to close the bus (wireReadDataByte). \n");
	exit(1);
    }
    return true;
